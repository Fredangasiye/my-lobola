// Uncle Wisdom API endpoint with real LLM integration
...
Please respond as Uncle Wisdom would - with warmth, wisdom, and cultural understanding. Keep your response under 200 words and focus on practical guidance that respects both tradition and modern values.`;

    // Use a free LLM service (Hugging Face Inference API)
    ...
    if (!response.ok) {
      ...
      console.log('LLM service unavailable, using fallback system');
      const generateResponse = (question, culturalGroup) => {
        ...
        if (questionLower.includes('cattle') || questionLower.includes('cow') || questionLower.includes('livestock') || questionLower.includes('animal') || questionLower.includes('how many')) {
          ...
          // For general questions, use a more specific response based on cultural group
    ...
    if (data && data[0] && data[0].generated_text) {
      ...
      const promptStart = fullResponse.indexOf('Question:');
      if (promptStart !== -1) {
        answer = fullResponse.substring(promptStart + question.length + 10).trim();
      } else {
        answer = fullResponse.replace(`${culturalContext}

You are speaking to someone who is seeking guidance about lobola (bride price) traditions. Provide thoughtful, culturally sensitive, and practical advice.

Question: ${question}

Please respond as Uncle Wisdom would - with warmth, wisdom, and cultural understanding. Keep your response under 200 words and focus on practical guidance that respects both tradition and modern values.`, '').trim();
      }
    } else {
